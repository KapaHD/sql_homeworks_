...
and Battles.date = (select max(date) from (Select Outcomes.ship, Outcomes.battle, Outcomes.result, Battles.date
from Outcomes full outer join Battles on Outcomes.battle = Battles.name join Classes on Classes.class=Outcomes.ship where result = 'sunk') as a).
